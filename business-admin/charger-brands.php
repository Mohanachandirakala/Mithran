...
if (!tableExists($conn, 'charger_brands')) {
    die('charger_brands table not found.');
}
            ...
            if ($handle !== false) {
                ...
                if ($headers === false) {
                    ...
                    foreach ($headers as $idx => $header) {
                        ...
                        if (in_array($cleanHeader, ['brandname', 'brand', 'name', 'chargername', 'chargerbrand'])) {
                            $brandNameIndex = $idx;
                        }
                    ...
                    if ($brandNameIndex === null) {
                        ...
                        try {
                            ...
                            while (($row = fgetcsv($handle, 0, $delimiter)) !== false) {
                                ...
                                $checkStmt = $conn->prepare("SELECT id FROM charger_brands WHERE brand_name = ? LIMIT 1");
                            ...
                            $importSuccess = "Successfully imported {$importSuccessCount} charger brands.";
                            if ($importSkipped > 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (!isset($_POST['action']) || $_POST['action'] !== 'bulk_import')) {
    ...
    if ($action === 'add') {
        ...
        if ($brand_name === '') {
            ...
            $stmt = $conn->prepare("SELECT id FROM charger_brands WHERE brand_name = ? LIMIT 1");
            if ($stmt) {
                ...
                if ($dup) {
                    $error = 'Charger brand already exists.';
                }
            }
        }
    ...
    if ($action === 'edit') {
        ...
        if ($id <= 0) {
            ...
            $stmt = $conn->prepare("SELECT id FROM charger_brands WHERE brand_name = ? AND id != ? LIMIT 1");
            if ($stmt) {
    ...
    if ($action === 'delete') {
        ...
        if ($id <= 0) {
            ...
            if (tableExists($conn, 'vehicle_models')) {
                $stmt = $conn->prepare("SELECT COUNT(*) AS total FROM vehicle_models WHERE charger_brand = (SELECT brand_name FROM charger_brands WHERE id = ? LIMIT 1)");
                if ($stmt) {
            ...
            if (tableExists($conn, 'vehicle_stock')) {
                $stmt = $conn->prepare("SELECT COUNT(*) AS total FROM vehicle_stock WHERE charger_brand = (SELECT brand_name FROM charger_brands WHERE id = ? LIMIT 1)");
                if ($stmt) {
                    $stmt->bind_param('i', $id);
                    $stmt->execute();
                    $row = $stmt->get_result()->fetch_assoc();
            ...
            if ($usedInModels > 0 || $usedInStock > 0) {
                $error = 'Cannot delete this charger brand because it is linked to vehicle models or stock.';
            } else {
...
if ($search !== '') {
    ...
    $where .= " AND cb.brand_name LIKE '%{$safe}%'";
}
...
/* -------------------------------------------------------
   SUMMARY
------------------------------------------------------- */
$totalBrands = getCount($conn, 'charger_brands');
$activeBrands = getCount($conn, 'charger_brands', 'status = 1');
$inactiveBrands = getCount($conn, 'charger_brands', 'status = 0');
...
$sql = "SELECT 
            ...
            (
                ...
                WHERE vm.charger_brand = cb.brand_name
            ) AS model_count,
            (
                SELECT COUNT(*)
                FROM vehicle_stock vs
                WHERE vs.charger_brand = cb.brand_name
            ) AS stock_count
        FROM charger_brands cb
        WHERE {$where}
    ...
    <div class="main-content">
        <div class="page-content">
            <div class="container-fluid">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <h4 class="mb-1">Charger Brands</h4>
                        <p class="text-muted mb-0">Manage charger brand master data</p>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-xl-4">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title mb-4">Add Charger Brand</h4>
                                ...
                                <form method="post">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="brand_name" class="form-control" placeholder="Enter charger brand name" required>
                                    </div>
                                ...
                                <form method="get" class="row g-3 mb-4">
                                    <div class="col-md-8">
                                        <input
                                            ...
                                            name="search"
                                            class="form-control"
                                            placeholder="Search charger brand..."
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped align-middle mb-0">
                                        ...
                                        <tbody>
                                                ...
                                                <?php $i = 1; foreach ($brands as $row): ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <form method="post" class="d-inline" onsubmit="return confirm('Delete this charger brand?');">
                                            ...
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="7" class="text-center text-muted py-4">No charger brands found.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
        ...
        <div class="modal-content">
            <div class="modal-header bg-success text-white">
                <h5 class="modal-title" id="bulkImportModalLabel">
                    <i class="ri-upload-cloud-line me-2"></i>Bulk Import Charger Brands
                </h5>
                ...
                <div class="modal-body">
                    ...
                    <?php if ($bulkResults !== null && $bulkResults['success'] > 0): ?>
                        <div class="alert alert-success">
                            ...
                            <strong><?php echo $bulkResults['success']; ?></strong> charger brands imported successfully!
                    ...
                    <p class="text-muted mb-3">
                        Upload a CSV file with charger brand names.
...
function downloadSample() {
    ...
    csvContent += "Delta,1\n";
    csvContent += "Mean Well,1\n";
    csvContent += "TDK Lambda,1\n";
    csvContent += "Meanwell,1\n";
    csvContent += "Eltek,1\n";
    csvContent += "Vertiv,1\n";
    csvContent += "Emerson,1\n";
    csvContent += "Cosel,1\n";
    ...
    link.setAttribute("download", "sample_charger_brands.csv");
