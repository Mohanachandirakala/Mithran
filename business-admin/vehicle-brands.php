            ...
            if ($handle !== false) {
                ...
                if ($headers === false) {
                    ...
                    foreach ($headers as $idx => $header) {
                        ...
                        if (in_array($cleanHeader, ['brandname', 'brand', 'name', 'brandnames'])) {
                            $brandNameIndex = $idx;
                        }
                    ...
                    // If no brand name column found, use first column
                    ...
                    if ($brandNameIndex === null) {
                        ...
                        try {
                            ...
                            while (($row = fgetcsv($handle, 0, $delimiter)) !== false) {
                                ...
                                $checkStmt = $conn->prepare("SELECT id FROM vehicle_brands WHERE brand_name = ? LIMIT 1");
                                ...
                                // Insert brand
                                $stmt = $conn->prepare("INSERT INTO vehicle_brands (brand_name, status, created_at) VALUES (?, ?, NOW())");
                                if ($stmt) {
                            ...
                            $importSuccess = "Successfully imported {$importSuccessCount} brands.";
                            if ($importSkipped > 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (!isset($_POST['action']) || $_POST['action'] !== 'bulk_import')) {
    ...
    if ($action === 'add') {
        ...
        if ($error === '') {
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    $success = 'Vehicle brand added successfully.';
                } else {
                    $error = 'Failed to add vehicle brand.';
                }
                $stmt->close();
            } else {
    ...
    if ($action === 'edit') {
        ...
        if ($id <= 0) {
            ...
            $stmt = $conn->prepare("SELECT id FROM vehicle_brands WHERE brand_name = ? AND id != ? LIMIT 1");
            if ($stmt) {
        ...
        if ($error === '') {
            $stmt = $conn->prepare("UPDATE vehicle_brands SET brand_name = ?, status = ? WHERE id = ? LIMIT 1");
            if ($stmt) {
    ...
    if ($action === 'delete') {
        ...
        if ($id <= 0) {
            ...
            if (tableExists($conn, 'vehicle_models')) {
                $stmt = $conn->prepare("SELECT COUNT(*) AS total FROM vehicle_models WHERE brand_id = ?");
                if ($stmt) {
            ...
            if ($usedCount > 0) {
                $error = 'Cannot delete this brand because vehicle models are linked to it.';
            } else {
                $stmt = $conn->prepare("DELETE FROM vehicle_brands WHERE id = ? LIMIT 1");
                if ($stmt) {
                    $stmt->bind_param('i', $id);
                    if ($stmt->execute()) {
...
if ($search !== '') {
    ...
    $where .= " AND vb.brand_name LIKE '%{$safe}%'";
}

if ($statusFilter !== '') {
    if ($statusFilter === 'active') {
        $where .= " AND vb.status = 1";
    } elseif ($statusFilter === 'inactive') {
        $where .= " AND vb.status = 0";
    }
}
...
/* -------------------------------------------------------
   SUMMARY
------------------------------------------------------- */
$totalBrands = getCount($conn, 'vehicle_brands');
$activeBrands = getCount($conn, 'vehicle_brands', 'status = 1');
$inactiveBrands = getCount($conn, 'vehicle_brands', 'status = 0');
    ...
    <div class="main-content">
        <div class="page-content">
            <div class="container-fluid">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <h4 class="mb-1">Vehicle Brands</h4>
                        <p class="text-muted mb-0">Manage vehicle brand master data</p>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-xl-4">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title mb-4">Add Vehicle Brand</h4>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped align-middle mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="6" class="text-center text-muted py-4">No vehicle brands found.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
        ...
        <div class="modal-content">
            <div class="modal-header bg-success text-white">
                <h5 class="modal-title" id="bulkImportModalLabel">
                    <i class="ri-upload-cloud-line me-2"></i>Bulk Import Brands
                </h5>
                ...
                <div class="modal-body">
                    ...
                    <?php if ($bulkResults !== null && $bulkResults['success'] > 0): ?>
                        <div class="alert alert-success">
                            ...
                            <strong><?php echo $bulkResults['success']; ?></strong> brands imported successfully!
                    ...
                    <p class="text-muted mb-3">
                        Upload a CSV file with brand names.
...
function downloadSample() {
    ...
    link.setAttribute("download", "sample_brands.csv");
