...
if ($editId > 0) {
    $stmt = $conn->prepare("SELECT id, brand_name, status FROM battery_brands WHERE id = ? LIMIT 1");
    if ($stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($brand_name === '') {
        ...
        if ($brand_id > 0) {
            $check = $conn->prepare("SELECT id FROM battery_brands WHERE brand_name = ? AND id != ? LIMIT 1");
            if ($check) {
                ...
                if ($exists) {
                    $error = 'Battery brand name already exists.';
                } else {
                    ...
                    if ($stmt) {
                        ...
                        $error = 'Database error while updating battery brand.';
                    }
                }
            } else {
                $error = 'Database error while checking battery brand.';
            }
        } else {
            $check = $conn->prepare("SELECT id FROM battery_brands WHERE brand_name = ? LIMIT 1");
            if ($check) {
                ...
                if ($exists) {
                    ...
                    if ($stmt) {
                        ...
                        $error = 'Database error while adding battery brand.';
                    }
                }
            } else {
                $error = 'Database error while checking battery brand.';
...
if (isset($_GET['delete']) && $_GET['delete'] !== '') {
    ...
    if ($deleteId > 0) {
        ...
        if ($stmt) {
            ...
            $error = 'Database error while deleting battery brand.';
        }
    }
}
...
$sql = "SELECT id, brand_name, status, created_at FROM battery_brands {$where} ORDER BY brand_name ASC";
...
$res = $conn->query("SELECT COUNT(*) AS total FROM battery_brands");
if ($res && $row = $res->fetch_assoc()) {
    $totalBrands = (int)$row['total'];
}

$res = $conn->query("SELECT COUNT(*) AS total FROM battery_brands WHERE status = 1");
if ($res && $row = $res->fetch_assoc()) {
    $activeBrands = (int)$row['total'];
}

$res = $conn->query("SELECT COUNT(*) AS total FROM battery_brands WHERE status = 0");
if ($res && $row = $res->fetch_assoc()) {
                        ...
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title mb-4">
                                    <?php echo $editBrand ? 'Edit Battery Brand' : 'Add Battery Brand'; ?>
                                </h4>
                                ...
                                <form method="post" action="">
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label">Battery Brand Name <span class="text-danger">*</span></label>
                                    ...
                                    <div class="d-grid gap-2">
                                        <button type="submit" class="btn btn-primary">
                                            <?php echo $editBrand ? 'Update Battery Brand' : 'Add Battery Brand'; ?>
                                        </button>

                                        <?php if ($editBrand): ?>
                                            <a href="battery-brands.php" class="btn btn-secondary">Cancel Edit</a>
                                        <?php endif; ?>
                                    </div>
                                </form>
                            </div>
                                ...
                                <form method="get" class="row g-3 align-items-end mb-4">
                                    <div class="col-md-8">
                                        ...
                                        <input type="text" name="search" class="form-control"
                                               placeholder="Search battery brand name"
                                    ...
                                    <div class="col-md-12">
                                        <div class="d-flex gap-2">
                                            ...
                                            <a href="battery-brands.php" class="btn btn-secondary">Reset</a>
                                        </div>
                                    </div>
                                </form>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover align-middle mb-0">
                                        ...
                                        <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <div class="d-flex flex-wrap gap-1">
                                                                <a href="battery-brands.php?edit=<?php echo (int)$row['id']; ?>" class="btn btn-primary btn-sm">Edit</a>
                                                                <a href="battery-brands.php?delete=<?php echo (int)$row['id']; ?>"
                                                                   class="btn btn-danger btn-sm"
                                                                   onclick="return confirm('Are you sure you want to delete this battery brand?');">
                                                                    Delete
                                                                </a>
                                                            </div>
                                                        </td>
                                            ...
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="5" class="text-center text-muted py-4">No battery brands found.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
