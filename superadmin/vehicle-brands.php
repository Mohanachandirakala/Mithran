...
if ($editId > 0) {
    $stmt = $conn->prepare("SELECT id, brand_name, status FROM vehicle_brands WHERE id = ? LIMIT 1");
    if ($stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($brand_name === '') {
        ...
        if ($brand_id > 0) {
            $check = $conn->prepare("SELECT id FROM vehicle_brands WHERE brand_name = ? AND id != ? LIMIT 1");
            if ($check) {
            ...
            $check = $conn->prepare("SELECT id FROM vehicle_brands WHERE brand_name = ? LIMIT 1");
            if ($check) {
...
if (isset($_GET['delete']) && $_GET['delete'] !== '') {
    ...
    if ($deleteId > 0) {
        $check = $conn->prepare("SELECT COUNT(*) AS total FROM vehicle_models WHERE brand_id = ?");
        if ($check) {
            ...
            if ($usedCount > 0) {
                $error = 'This brand is already used in vehicle models, so it cannot be deleted.';
            } else {
...
$res = $conn->query("SELECT COUNT(*) AS total FROM vehicle_brands");
if ($res && $row = $res->fetch_assoc()) {
    $totalBrands = (int)$row['total'];
}

$res = $conn->query("SELECT COUNT(*) AS total FROM vehicle_brands WHERE status = 1");
if ($res && $row = $res->fetch_assoc()) {
    $activeBrands = (int)$row['total'];
}

$res = $conn->query("SELECT COUNT(*) AS total FROM vehicle_brands WHERE status = 0");
if ($res && $row = $res->fetch_assoc()) {
                            ...
                            <div class="card-body">
                                ...
                                <form method="post" action="">
                                    ...
                                    <div class="d-grid gap-2">
                                        ...
                                        <?php if ($editBrand): ?>
                                            <a href="vehicle-brands.php" class="btn btn-secondary">Cancel Edit</a>
                                        <?php endif; ?>
                                    </div>
                                </form>
                            </div>
                                ...
                                <form method="get" class="row g-3 align-items-end mb-4">
                                    ...
                                    <div class="col-md-3">
                                        <div class="d-flex gap-2">
                                            ...
                                            <a href="vehicle-brands.php" class="btn btn-secondary">Reset</a>
                                        </div>
                                    </div>
                                </form>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <div class="d-flex flex-wrap gap-1">
                                                                <a href="vehicle-brands.php?edit=<?php echo (int)$row['id']; ?>" class="btn btn-primary btn-sm">Edit</a>
                                                                <a href="vehicle-brands.php?delete=<?php echo (int)$row['id']; ?>"
