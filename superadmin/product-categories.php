...
if ($editId > 0) {
    $stmt = $conn->prepare("SELECT id, category_name FROM product_categories WHERE id = ? LIMIT 1");
    if ($stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($category_name === '') {
        ...
        if ($category_id > 0) {
            $check = $conn->prepare("SELECT id FROM product_categories WHERE category_name = ? AND id != ? LIMIT 1");
            if ($check) {
            ...
            $check = $conn->prepare("SELECT id FROM product_categories WHERE category_name = ? LIMIT 1");
            if ($check) {
...
if (isset($_GET['delete']) && $_GET['delete'] !== '') {
    ...
    if ($deleteId > 0) {
        ...
        if ($check) {
            ...
            if ($usedCount > 0) {
                $error = 'This category is already used in products, so it cannot be deleted.';
            } else {
...
if ($search !== '') {
    $where .= " AND pc.category_name LIKE ? ";
    $params[] = "%{$search}%";
    $types .= "s";
}
...
$res = $conn->query("SELECT COUNT(*) AS total FROM product_categories");
                            ...
                            <div class="card-body">
                                ...
                                <form method="post" action="">
                                    ...
                                    <div class="d-grid gap-2">
                                        ...
                                        <?php if ($editCategory): ?>
                                            <a href="product-categories.php" class="btn btn-secondary">Cancel Edit</a>
                                        <?php endif; ?>
                                    </div>
                                </form>
                            </div>
                                ...
                                <form method="get" class="row g-3 align-items-end mb-4">
                                    ...
                                    <div class="col-md-4">
                                        <div class="d-flex gap-2">
                                            ...
                                            <a href="product-categories.php" class="btn btn-secondary">Reset</a>
                                        </div>
                                    </div>
                                </form>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover align-middle mb-0">
                                        ...
                                        <tbody>
                                            <?php if (!empty($categories)): ?>
                                                ...
                                                <?php foreach ($categories as $row): ?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo number_format((int)$row['product_count']); ?></td>
                                                        <td>
                                                            <div class="d-flex flex-wrap gap-1">
                                                                <a href="product-categories.php?edit=<?php echo (int)$row['id']; ?>" class="btn btn-primary btn-sm">Edit</a>
                                                                <a href="product-categories.php?delete=<?php echo (int)$row['id']; ?>"
                                            ...
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="4" class="text-center text-muted py-4">No product categories found.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
